      ...
      {noData ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? 10 : 14, marginBottom: isMobile ? 32 : 40 }}>
            <StatCard label="Total plays"   value={stats?.total_plays   ?? 0} icon="▶" />
            <StatCard label="Today"         value={stats?.plays_today   ?? 0} icon="☀" />
            <StatCard label="This week"     value={stats?.plays_week    ?? 0} icon="📅" />
            <StatCard label="Unique tracks" value={stats?.unique_tracks ?? 0} icon="🎵" />
          </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {topArtists.map((a, i) => (
                    <div
                      key={a.artist_id}
                      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.05)' }}
                    >
                      {/* Rank badge */}
                      ...
                      <div style={{ width: 48, height: 3, borderRadius: 2, background: 'rgba(255,255,255,.08)', flexShrink: 0, overflow: 'hidden' }}>
                        <div style={{
                          ...
                          width: `${Math.round((a.play_count / (topArtists[0]?.play_count || 1)) * 100)}%`,
                        }} />
                      </div>
                    </div>
                  ))}
...
function StatCard({ label, value, icon }: { label: string; value: number; icon: string }) {
  return (
    <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 12, padding: '16px 18px' }}>
